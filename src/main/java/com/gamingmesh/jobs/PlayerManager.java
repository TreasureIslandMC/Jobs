...
//    }
	...
	public enum BoostOf {
		MC_MMO, PET_PAY, NEAR_SPAWNER, PERMISSION, GLOBAL, DYNAMIC, ITEM, AREA
	}
	...
	public Boost getFinalBonus(JobsPlayer player, Job prog, Entity ent, LivingEntity victim, boolean force, boolean getall) {
		...
		if (HookManager.getMcMMOManager().mcMMOPresent || HookManager.getMcMMOManager().mcMMOOverHaul)
			boost.add(BoostOf.MC_MMO, new BoostMultiplier().add(HookManager.getMcMMOManager().getMultiplier(player.getPlayer())));
		...
		if (ent != null && (ent instanceof Tameable)) {
			...
			if (t.isTamed() && t.getOwner() instanceof Player) {
				...
				if (amount != null)
					boost.add(BoostOf.PET_PAY, new BoostMultiplier().add(amount));
			}
		}
		...
		if (victim != null && victim.hasMetadata(getMobSpawnerMetadata())) {
			...
			if (amount != null)
				boost.add(BoostOf.NEAR_SPAWNER, new BoostMultiplier().add(amount));
		}
